...
export default function GeneratorResult({ result, onAccept, onDiscard, costModeEnabled, theme }) {
  if (!result) return null
  const { totals, groups, area } = result

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-[var(--c-bg-panel)] border border-[var(--c-border)] rounded-xl shadow-2xl w-[640px] max-h-[85vh] flex flex-col">
        ...
        <div className="px-5 py-4 border-t border-[var(--c-border)] flex gap-3 justify-end shrink-0">
          ...
          <button onClick={onAccept} className="text-xs px-4 py-1.5 rounded bg-teal-700 hover:bg-teal-600 text-[var(--c-text-strong)] transition-colors">
            Accept changes
          </button>
        </div>
      </div>
