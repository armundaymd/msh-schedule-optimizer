...
function StatRow({ label, current, scenario, format = String, deltaFormat = format }) {
  ...
  const deltaColor = delta === 0 ? 'text-slate-500' : delta > 0 ? 'text-amber-400' : 'text-green-400'
  return (
    <tr className="text-xs">
      <td className="py-1 pr-3 text-slate-400">{label}</td>
      <td className="py-1 pr-3 text-slate-200 text-right">{format(current)}</td>
      <td className="py-1 pr-3 text-slate-200 text-right">{format(scenario)}</td>
...
export default function ScenariosPanel({
  ...
  onDeleteScenario, onResetToScenario, currentShiftsByDay, costRates,
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-[560px] max-h-[80vh] flex flex-col">
        <div className="px-5 py-4 border-b border-slate-700 shrink-0 flex items-center justify-between">
          <div className="text-sm font-semibold text-slate-100">📊 Saved scenarios</div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-200 text-sm">✕</button>
        </div>

        <div className="flex-1 overflow-y-auto px-5 py-3 space-y-2">
          {scenarios.length === 0 ? (
            <div className="text-xs text-slate-500">No scenarios saved yet. Use "Save scenario" in the top bar to snapshot the current week.</div>
          ) : (
            scenarios.map(sc => {
              ...
              return (
                <div key={sc.id} className="border border-slate-800 rounded">
                  <button
                  ...
                  >
                    <div className="text-left">
                      ...
                      <div className="text-[10px] text-slate-500">
                        {new Date(sc.created_at).toLocaleString()} · {hours.toFixed(1)} att-hrs · {currency(cost)} · {shiftCount} shifts
                      </div>
                    </div>
                      ...
                      <div className="flex gap-2 pt-1">
                        ...
                        <button
                          ...
                          className={`text-xs px-3 py-1 rounded transition-colors ${
                            comparing ? 'bg-orange-800 hover:bg-orange-700 text-orange-100' : 'bg-slate-700 hover:bg-slate-600 text-slate-200'
                          }`}
                        >
                        ...
                        <button
                          ...
                          className="text-xs px-3 py-1 rounded bg-slate-700 hover:bg-red-900 text-slate-300 hover:text-red-200 transition-colors ml-auto"
                        >
                          Delete
                        </button>
                      </div>
        ...
        <div className="px-5 py-3 border-t border-slate-700 shrink-0 text-right">
